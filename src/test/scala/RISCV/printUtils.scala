...
object PrintUtils {
  ...
  implicit val ExecutionEventFancy = new Fancy[ExecutionEvent]{
    def s(a: ExecutionEvent): fansi.Str = a match {
      ...
      case RegUpdate(reg, word) => Str(s"R(${reg.show}) <- ${word.hs}")
      case MemWrite(addr, word) => fansi.Color.Blue(s"M[${addr.show}] <- ${word.hs}")
      case MemRead(addr, word)  => fansi.Color.Red(f"M[${addr.show}] -> ${word.hs}")
